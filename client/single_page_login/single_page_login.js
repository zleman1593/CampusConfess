...
Template.singlePageSignUp.helpers({
  ...
  'formFields': function() {
    if(!Accounts._options['forbidClientAccountCreation'])
      ...
      default:
        // 'EMAIL_ONLY'
        html = '<input name="email" id="email" type="text" class="form-control" placeholder="School (.edu) Email" required autofocus>';
      }
...
var Alert = {
  ...
  emailConfirm: function (msg, t) {
    t.find('#single-page-login-alert').innerHTML = '<div class="alert alert-success" id="single-page-login-alert-msg">' + msg.reason + '</div>';
     t.find('.form-signin').remove();
          t.find('#new-account').remove();
          t.find('#forgot-password').remove();
     

  },
};

UI.registerHelper('getProfileImage', function(){
